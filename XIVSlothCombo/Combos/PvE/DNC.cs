            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is Cascade)
                {
                    ...
                    if (Config.DNC_SimpleST_Dances[0] &&
                        ActionReady(StandardStep) && !HasEffect(Buffs.TechnicalFinish) && IsOffCooldown(StandardStep))
                    {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is Cascade)
                {
                    ...
                    if (IsEnabled(CustomComboPreset.DNC_AdvST_SS) && Config.DNC_AdvST_SS_Options == 2 &&
                        !InCombat() && ActionReady(StandardStep) && IsOffCooldown(TechnicalStep) && IsOffCooldown(StandardStep) && !Gauge.IsDancing)
                        return StandardStep;
                    #endregion
                    ...
                    if (IsEnabled(CustomComboPreset.DNC_AdvST_SS) && Config.DNC_AdvST_SS_Options == 2 &&
                        ActionReady(StandardStep) && IsOffCooldown(StandardStep) && !HasEffect(Buffs.TechnicalFinish))
                    {
        ...
        internal class DNC_SimpleAoE : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is Windmill)
                {
                    ...
                    if (Config.DNC_SimpleAoE_Dances[0] && ActionReady(StandardStep) && IsOffCooldown(TechnicalStep) && IsOffCooldown(StandardStep) && !InCombat())
                        return StandardStep;
                    #endregion
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is Windmill)
                {
                    ...
                    if (IsEnabled(CustomComboPreset.DNC_AdvAoE_SS) && Config.DNC_AdvAoE_SS_Options == 2 &&
                        ActionReady(StandardStep) && IsOffCooldown(TechnicalStep) && !InCombat() && IsOffCooldown(StandardStep))
                        return StandardStep;
                    #endregion
